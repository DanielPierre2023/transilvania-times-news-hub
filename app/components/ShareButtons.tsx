...
export default function ShareButtons({ url, title, summary = '' }: ShareButtonsProps) {
  ...
  const es  = encodeURIComponent(summary.substring(0, 200))

  const btnClass = (extra: string) =>
    `inline-flex items-center gap-1.5 px-3 py-1.5 border border-foreground/20 text-foreground/70 text-[11px] font-sans font-bold uppercase tracking-wider transition-all duration-200 hover:text-white ${extra}`
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 my-8 py-6 border-t border-b border-foreground/10">
      <span className="text-[11px] font-sans font-bold uppercase tracking-widest text-muted-foreground mr-2">
        ...
        className={btnClass('hover:bg-[#1877F2] hover:border-[#1877F2]')}
      >
        ...
        className={btnClass('hover:bg-black hover:border-black')}
      >
        <svg viewBox="0 0 24 24" className="w-4 h-4 fill-current">
        ...
        href={`https://www.linkedin.com/shareArticle?mini=true&url=${eu}&title=${et}&summary=${es}`}
        target="_blank"
        rel="noopener noreferrer"
        aria-label="Distribuie pe LinkedIn"
        className={btnClass('hover:bg-[#0A66C2] hover:border-[#0A66C2]')}
      >
        <svg viewBox="0 0 24 24" className="w-4 h-4 fill-current">
        ...
        className={btnClass('hover:bg-[#25D366] hover:border-[#25D366]')}
      >
        <svg viewBox="0 0 24 24" className="w-4 h-4 fill-current">
      ...
      <button
        ...
        className={`inline-flex items-center gap-1.5 px-3 py-1.5 border transition-all duration-200 text-[11px] font-sans font-bold uppercase tracking-wider ${
          copied
