...
import {
  PlaneTakeoff, PlaneLanding, Search, RefreshCw, Plane, Twitter, Facebook, Share2,
} from 'lucide-react'
    ...
    <tr className="border-b border-foreground/[0.06] hover:bg-foreground/[0.02]">
      ...
      <td className="px-4 py-3 text-right pr-4 hidden sm:table-cell whitespace-nowrap">
        <div className="inline-flex items-center gap-2 text-muted-foreground">
          <a aria-label="X / Twitter" title="X" target="_blank" rel="noopener noreferrer"
            href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(shareText)}&url=${encodeURIComponent(SITE)}`}
            className="hover:text-brand-red transition-colors"><Twitter className="w-3.5 h-3.5" /></a>
          <a aria-label="Facebook" title="Facebook" target="_blank" rel="noopener noreferrer"
            ...
            className="hover:text-brand-red transition-colors"><Facebook className="w-3.5 h-3.5" /></a>
          <ShareNative text={shareText} />
        </div>
      </td>
    </tr>
  )
...
function ShareNative({ text }: { text: string }) {
  return (
    <button aria-label="Share" title="Share"
      onClick={() => {
        const nav = navigator as Navigator & { share?: (d: { text: string; url: string }) => Promise<void> }
        if (nav.share) nav.share({ text, url: SITE }).catch(() => {})
        else window.open(`https://wa.me/?text=${encodeURIComponent(text + ' ' + SITE)}`, '_blank', 'noopener')
      }}
      className="hover:text-brand-red transition-colors"><Share2 className="w-3.5 h-3.5" /></button>
  )
}
