...
export default async function HomePageEN() {
  ...
  const { data, error } = await supabase
    .from('blog_posts')
    .select(
      'id, slug, title_ro, title_en, category, subcategory, cover_image, excerpt_ro, excerpt_en, summary_ro, summary_en, author_name, published_at'
    )
    .eq('status', 'published')
  ...
  function getTitle(p: Post) {
    return p.title_en || p.title_ro || ''
  }
  function getExcerpt(p: Post) {
    return p.excerpt_en || p.excerpt_ro || ''
  }
  ...
  return (
    <div className="container mx-auto max-w-7xl px-4 py-8 space-y-10">
      ...
      {(leftColPosts.length > 0 ||
        centerColPosts.length > 0 ||
        rightColPosts.length > 0) && (
