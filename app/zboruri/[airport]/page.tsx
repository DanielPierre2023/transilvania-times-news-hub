...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = `Zboruri ${m.short} (${code}) — Plecări și Sosiri în timp real`
  const description = AIRPORT_SEO[code].ro.intro.slice(0, 155)
  return {
    title,
    description,
    alternates: {
      canonical: `${SITE_URL}/zboruri/${slug}/`,
      languages: { 'ro-RO': `${SITE_URL}/zboruri/${slug}/`, 'en': `${SITE_URL}/en/zboruri/${slug}/` },
    },
    openGraph: { title, description, url: `${SITE_URL}/zboruri/${slug}/`, type: 'website' },
  }
}
...
async function loadInitial(): Promise<{ flights: FlightRow[]; today: string }> {
  ...
  const { data } = await supabase
    ...
    .select('airport, direction, flight_date, flight_no, airline, city, aircraft, scheduled_time, estimated_time, status, status_raw, is_charter, source_url, updated_at')
...
export default async function AirportZboruriPage({ params }: PageProps) {
  const { airport } = await params
  const code = SLUG_TO_AIRPORT[airport]
  if (!code) notFound()
  ...
  const m = AIRPORTS[code]
  const slug = AIRPORT_SLUG[code]
  const copy = AIRPORT_SEO[code].ro
  const { flights, today } = await loadInitial()

  const ld = [
    {
      '@context': 'https://schema.org', '@type': 'WebPage',
      name: `Zboruri ${m.short} (${code})`, description: copy.intro,
      url: `${SITE_URL}/zboruri/${slug}/`, inLanguage: 'ro-RO',
    ...
    <>
      ...
      <div className="max-w-7xl mx-auto border-x border-foreground/10">
        <div className="px-4 sm:px-6 pt-8">
          <nav className="font-sans text-[11px] text-muted-foreground mb-4">
            <Link href="/zboruri/" className="hover:text-brand-red">Zboruri</Link> <span className="mx-1">/</span> {m.short}
          </nav>
          <h1 className="font-serif text-2xl md:text-3xl font-bold text-foreground leading-tight mb-3">
            Zboruri {m.short} <span className="font-mono text-muted-foreground">({code})</span>
          </h1>
      ...
      <div className="max-w-7xl mx-auto border-x border-foreground/10">
        <div className="px-4 sm:px-6 py-10 max-w-3xl">
          <h2 className="font-serif text-xl font-bold text-foreground mb-5">Întrebări frecvente</h2>
          ...
          <div className="mt-8 flex flex-wrap gap-2">
            {(['cluj', 'targu-mures', 'sibiu'] as const).filter(s => s !== slug).map(s => (
              <Link key={s} href={`/zboruri/${s}/`} className="font-sans text-[12px] font-bold uppercase tracking-wider text-brand-red border border-brand-red/20 px-3 py-1.5 hover:bg-brand-red/[0.06] transition-colors">
