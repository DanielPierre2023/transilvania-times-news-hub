...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const boardHref = `${SITE_URL}/zboruri/${AIRPORT_SLUG[p.airport]}/`
  ...
  const label = statusLabel(f, 'ro')
  const bits = [
    `${p.direction === 'departure' ? 'Plecare' : 'Sosire'} ${dir}`,
    `Programat: ${dateHeading(f.flight_date, 'ro')} ${sched}`,
  ]
  if (est && est !== sched) bits.push(`Estimat: ${est}`)
  if (f.gate) bits.push(`Poarta ${f.gate}`)
  ...
  return {
    ...
    openGraph: { title, description, url: `${SITE_URL}/zboruri/f/${slug}/`, type: 'website', siteName: 'Transilvania Times' },
...
export default async function FlightPermalinkPage({ params }: PageProps) {
  ...
  return (
    <>
      <meta httpEquiv="refresh" content={`0; url=/zboruri/${AIRPORT_SLUG[p.airport]}/`} />
      <FlightBoard initialFlights={flights} initialToday={today} initialLang="ro" initialAirport={p.airport} />
    </>
  )
}
