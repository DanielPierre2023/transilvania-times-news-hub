...
function assemble(
  ...
  const facebook = {
    hooks: fbHooks,
    text: dedupeSentences([fbHook, '', fbBody, fbQuestion ? '\n' + fbQuestion : '', '', tagsFor(SPEC.facebook.tags)])
  ...
  const instagram = {
    ...
    caption: dedupeSentences([igHooks[0] || clampWords(lead.title, SPEC.instagram.hook), '', igCaption, '',
  ...
  const linkedin = {
    hook: liHook,
    text: dedupeSentences([liHook, '', clampWords(str(li.body), SPEC.linkedin.body), '',
