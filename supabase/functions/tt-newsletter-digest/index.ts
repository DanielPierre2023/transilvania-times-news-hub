...
async function generateEditorialIntro(lang: 'ro' | 'en', topArticles: ArticleSummary[]): Promise<string> {
  ...
  const system = lang === 'ro'
    ? `Ești redactorul-șef al Transilvania Times. Scrii UN SINGUR paragraf scurt (80–100 de cuvinte) la începutul digest-ului săptămânal. Identifici tema dominantă a săptămânii și o conturezi în cuvintele tale, fără să rezumi articolele individual. Voce: editorial sobru, nu publicitar. Fără adjective de impact ("șocant", "remarcabil"). Fără "în această săptămână" repetat. Începi direct cu o observație. Ritm uman: alternează o frază foarte scurtă (2–5 cuvinte) cu una lungă; evită două fraze de aceeași lungime la rând. Verbe vii, nu "este important". Nu încheia rezumând. Doar stil — nu inventa fapte. Diacritice obligatorii.`
    : `You are the editor of Transilvania Times. Write ONE short paragraph (80–100 words) opening the weekly digest. Identify the dominant theme of the week and outline it in your own words — do not summarize articles individually. Voice: sober editorial, not promotional. No impact adjectives ("shocking", "remarkable"). No repeated "this week". Open with a direct observation. Human rhythm: alternate a very short sentence (2-5 words) with a long one; avoid two same-length sentences in a row. Live verbs, not "it is important". Do not end by summarizing. Style only — never invent facts.`
